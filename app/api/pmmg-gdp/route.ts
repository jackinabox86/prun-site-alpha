import { NextResponse } from "next/server";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";
export const revalidate = 0;

const GITHUB_RAW =
  "https://raw.githubusercontent.com/PMMG-Products/pmmg-products.github.io/main/reports/data";
const GITHUB_API =
  "https://api.github.com/repos/PMMG-Products/pmmg-products.github.io/contents/reports/data";

const MONTH_ABBRS = [
  "jan", "feb", "mar", "apr", "may", "jun",
  "jul", "aug", "sep", "oct", "nov", "dec",
];
const MONTH_NAMES = [
  "January", "February", "March", "April", "May", "June",
  "July", "August", "September", "October", "November", "December",
];

function codeToYearMonth(code: string): string {
  const abbr = code.slice(0, 3);
  const yy = code.slice(3);
  const idx = MONTH_ABBRS.indexOf(abbr);
  const mm = String(idx + 1).padStart(2, "0");
  return `20${yy}-${mm}`;
}

function formatMonthLabel(code: string): string {
  const abbr = code.slice(0, 3);
  const yy = code.slice(3);
  const idx = MONTH_ABBRS.indexOf(abbr);
  return idx >= 0 ? `${MONTH_NAMES[idx]} 20${yy}` : code;
}

interface GitHubFileEntry {
  name: string;
  type: string;
}

interface ProdDataFile {
  [ticker: string]: {
    amount: number;
    volume: number;
    profit: number;
    consumed?: number;
  };
}

export interface GDPPoint {
  monthCode: string;
  monthLabel: string;
  totalVolume: number;
}

export interface GDPApiResponse {
  data: GDPPoint[];
  error?: string;
}

export async function GET() {
  try {
    const dirRes = await fetch(GITHUB_API, {
      headers: { Accept: "application/vnd.github+json" },
      cache: "no-store",
    });
    if (!dirRes.ok) {
      return NextResponse.json(
        { error: "Could not retrieve file listing from GitHub." } satisfies Partial<GDPApiResponse>,
        { status: 502 }
      );
    }

    const entries: GitHubFileEntry[] = await dirRes.json();
    const monthCodes: string[] = [];
    for (const entry of entries) {
      const match = entry.name.match(/^prod-data-([a-z]+\d{2})\.json$/);
      if (match) monthCodes.push(match[1]);
    }

    // Sort oldest first for the chart
    monthCodes.sort((a, b) => codeToYearMonth(a).localeCompare(codeToYearMonth(b)));

    if (monthCodes.length === 0) {
      return NextResponse.json(
        { error: "No prod-data files found." } satisfies Partial<GDPApiResponse>,
        { status: 404 }
      );
    }

    // Fetch all months in parallel
    const fetches = await Promise.all(
      monthCodes.map((code) =>
        fetch(`${GITHUB_RAW}/prod-data-${code}.json`, { cache: "no-store" })
      )
    );

    const data: GDPPoint[] = await Promise.all(
      fetches.map(async (res, i) => {
        const code = monthCodes[i];
        if (!res.ok) {
          return { monthCode: code, monthLabel: formatMonthLabel(code), totalVolume: 0 };
        }
        const prodData: ProdDataFile = await res.json();
        const totalVolume = Object.values(prodData).reduce(
          (sum, entry) => sum + (entry.volume ?? 0),
          0
        );
        return { monthCode: code, monthLabel: formatMonthLabel(code), totalVolume };
      })
    );

    return NextResponse.json({ data } satisfies GDPApiResponse);
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown error";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
