...
export async function buildReport(opts: {
  ...
  // Only compute expanded rows when requested
  let bestRows: [string, number | string][] | undefined;
  if (includeRows) {
    bestRows = buildScenarioRows(best.o, 0, best.capacity, expand).rows.slice();
    // Do NOT push Input Payback here — engine.ts already adds ROI & Input Payback on the root.
  }

  // Numeric metrics on the raw object
  ...
  // Top 5 summary: include ROI (omit input payback)
  ...
  return {
    ...
    best: {
      ...bestRaw,
      ...(includeRows ? { rows: bestRows } : {}), // <-- only when requested
    },
    top5,
  };
}
